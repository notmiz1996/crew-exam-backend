...
def submit_answer(request, exam_id, pq_id):
    ...
    try:
        epq = ExamPaperQuestion.objects.get(id=pq_id, exam_paper=exam_paper)
    except ExamPaperQuestion.DoesNotExist:
        return Response(error_response(1006, '题目不存在'))

    selected = request.data.get('selected_answer', None)
    ...
    answer, created = Answer.objects.update_or_create(
        ...
        defaults={'selected_answer': selected_value, 'answered_at': timezone.now()},
    )
